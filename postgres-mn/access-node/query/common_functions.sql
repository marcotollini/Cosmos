...
CREATE TYPE timescale_snapshot_setup_typ AS (column_name text, partition_column text, chunk_time_interval bigint, column_name_info text, partition_column_info text, chunk_time_interval_info bigint);
...
BEGIN
    IF _original_table_name = 'event_init' THEN SELECT 'timestamp_arrival', 'bmp_router', 7*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    ELSIF _original_table_name = 'event_log_init' THEN SELECT 'timestamp', 'bmp_router', 7*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    ELSIF _original_table_name = 'event_peer_down' THEN SELECT 'timestamp_arrival', 'bmp_router', 7*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    ELSIF _original_table_name = 'event_peer_up' THEN SELECT 'timestamp_arrival', 'bmp_router', 7*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    ELSIF _original_table_name = 'event_route_monitor' THEN SELECT 'timestamp_arrival', 'bmp_router', 3*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    ELSIF _original_table_name = 'event_stats' THEN SELECT 'timestamp_arrival', 'bmp_router', 1*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    ELSIF _original_table_name = 'event_log_close' THEN SELECT 'timestamp', 'bmp_router', 7*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    ELSIF _original_table_name = 'event_term' THEN SELECT 'timestamp_arrival', 'bmp_router', 7*24*60*60 INTO s.column_name, s.partition_column, s.chunk_time_interval; SELECT 'timestamp_start', 7*24*60*60*1000000::bigint INTO s.column_name_info, s.chunk_time_interval_info; RETURN s;
    END IF;
END;
$$;
